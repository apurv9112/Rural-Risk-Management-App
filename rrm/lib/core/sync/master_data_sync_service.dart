...
class MasterDataSyncService {
  ...
  Future<void> processSyncPayload(
    String category,
    List<MasterDataSyncItem> payload,
  ) async {
    ...
    if (payload.isNotEmpty) {
      ...
      for (var item in payload) {
        if (latestServerUpdated == null ||
            item.serverUpdatedAt.compareTo(latestServerUpdated) > 0) {
  ...
  Future<void> _updateSyncCursor(
    ...
    await database.transaction((txn) async {
      ...
      if (existing.isEmpty) {
        ...
        await txn.update(
          'master_data_sync_state',
          {'last_server_updated_at': lastServerUpdatedAt, 'last_sync_at': now},
          where: 'category = ?',
          whereArgs: [category],
        );
      }
