...
abstract class MasterDataSyncClient {
  Future<Map<String, dynamic>> fetchPage(String category, int page, int pageSize);
}
...
class MasterDataSyncCoordinator {
    ...
    try {
      ...
      if (state != null) {
        if (state['sync_status'] == 'IN_PROGRESS') {
          ...
          if (age > 15) {
            debugPrint('Recovering stale IN_PROGRESS sync for $category (Age: $age mins)');
            currentPage = (state['current_page'] as int?) ?? 1;
            totalPages = (state['total_pages'] as int?) ?? 1;
            syncSessionId = (state['sync_session_id'] as String?) ?? syncSessionId;
            startedAt = (state['started_at'] as String?) ?? startedAt;
          } else {
            debugPrint('Valid IN_PROGRESS sync for $category detected. Aborting duplicate.');
            return;
          }
      ...
      while (currentPage <= totalPages) {
        ...
        try {
          ...
          final lastServerUpdatedAt = response['last_server_updated_at'] as String?;
          ...
          String? completedAt = status == 'COMPLETED' ? DateTime.now().toIso8601String() : null;
          ...
          final stateUpdate = {
            ...
            'total_pages': totalPages,
            'sync_status': status,
            'last_server_updated_at': lastServerUpdatedAt ?? state?['last_server_updated_at'],
            'started_at': startedAt,
          ...
          if (status == 'COMPLETED') {
            stateUpdate['last_successful_sync_at'] = DateTime.now().toIso8601String();
          }
  ...
  Future<void> _updateState(String category, Map<String, dynamic> stateUpdate) async {
